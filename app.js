...
(function () {
  ...
  /* ---------------------------------------------------------
     ...
     --------------------------------------------------------- */
  function calcSplit(price, tips, rules) {
    ...
    if (price <= r.threshold) {
      me = Math.min(price, r.keepBelow);
    } else {
      me = price * r.pctAbove / 100;
    }
    let boss = price - me;
    ...
    return { me: round2(me), boss: round2(boss) };
  }
  ...
  function cutRow(c) {
    ...
    return '<button class="cut-row" data-action="edit-cut" data-id="' + esc(c.id) + '">' +
      ...
      '<div class="cut-right"><div class="cut-me">Tu: ' + money(c.me) + '</div>' +
      (c.boss > 0 ? '<div class="cut-boss">Șef: ' + money(c.boss) + '</div>' : '') +
      '</div></button>';
  }
  ...
  function ruleHelpText() {
    ...
    return 'Tuns de până la ' + fmt(r.threshold) + ' lei: tu păstrezi ' + fmt(r.keepBelow) + ' lei (sau tot prețul, dacă e mai mic), restul merge la șef. ' +
      'Peste ' + fmt(r.threshold) + ' lei: ' + fmt(r.pctAbove) + '% tu, ' + fmt(100 - r.pctAbove) + '% șef. ' +
      'Din tips rămâne ' + fmt(r.tipsPct) + '% la tine. ' +
      'Regula se aplică tunsurilor noi sau editate; cele vechi rămân cum au fost salvate.';
  }
  ...
  // Împărțirea folosită la salvare: dacă la editare prețul și tips-ul nu s-au schimbat,
  // se păstrează împărțirea originală (istoricul nu se rescrie).
  function splitForSave(price, tips) {
    const o = ui.cutForm && ui.cutForm.original;
    if (o && o.price === round2(price) && o.tips === round2(tips)) return { me: o.me, boss: o.boss };
    return calcSplit(price, tips);
  }
  ...
  function updateCutPreview() {
    ...
    const sp = splitForSave(price, tips);
    $('#cutPreview').innerHTML =
      '<span>Total client: <b>' + money(price + tips) + '</b></span>' +
      '<span>Tu: <b>' + money(sp.me) + '</b></span>' +
      '<span>Șef: <b>' + money(sp.boss) + '</b></span>';
  }
  ...
  function validateBackup(raw) {
    ...
    const cuts = raw.cuts.map((c, i) => {
      ...
      if (seen[id]) id = uid();
      seen[id] = true;
      let me = Number(c.me), boss = Number(c.boss);
      if (!isFinite(me) || !isFinite(boss)) {
        const sp = calcSplit(price, tips, settings);
        me = sp.me; boss = sp.boss;
      }
      return {
        id: id,
  ...
  window.__jb = { calcSplit: calcSplit, state: state, fmt: fmt, summarize: summarize };
